...
def save_bests(it, Data, Params, EXAMPLE):
    ...
    sio.savemat(full_path, {
        ...
        'x_opt': Data.x_opt.detach().cpu().numpy(),
        'f_opt': Data.f_opt.detach().cpu().numpy(),
        'layer_sizes': np.array(Data.layer_sizes),
        'train_x': np.array(Data.train_x.detach().cpu().numpy()),
        'val_x': np.array(Data.val_x.detach().cpu().numpy()),
        'train_g': np.array(Data.train_g.detach().cpu().numpy()),
        'val_g': np.array(Data.val_g.detach().cpu().numpy()),
        'x_max': np.array(Data.x_max.detach().cpu().numpy()),
        'x_min': np.array(Data.x_min.detach().cpu().numpy()),
        })
